...
function bv_qt_initialize_vars
{
    ...
    if [[ $USE_SYSTEM_QT != "yes" ]]; then
        ...
        if [[ $DO_QT510 == "yes" || $DO_QT514 == "yes" ]]; then
...
function bv_qt_info
{
    ...
    if [[ "$DO_QT510" == "yes" ]] ; then
        ...
        export QT_URL=${QT_URL:-"https://download.qt.io/archive/qt/${QT_SHORT_VERSION}/${QT_VERSION}/submodules"}
...
function build_qt6_tools
{
    ...
    copts="-DQt6_DIR:PATH=${QT_INSTALL_DIR}/lib/cmake/Qt6"
    copts="${copts} -DCMAKE_INSTALL_PREFIX:PATH=${QT_INSTALL_DIR}"
    ...
    env PATH="${qt6_path}" CC="${C_COMPILER}" CXX="${CXX_COMPILER}"  \
        ${CMAKE_COMMAND} ${copts} ../${QT_TOOLS_SOURCE_DIR}
...
function build_qt6_svg
{
    ...
    env PATH="${qt6_path}" CC="${C_COMPILER}" CXX="${CXX_COMPILER}"  \
        ${CMAKE_COMMAND} ${copts} ../${QT_SVG_SOURCE_DIR}
...
function bv_qt_is_enabled
{
    ...
    if [[ $DO_QT == "yes" || $DO_QT510 == "yes" || $DO_QT514 == "yes" ]]; then
        return 1    
    fi
    return 0
}
...
function bv_qt_build
{
    ...
    if [[ $DO_QT510 == "yes" || $DO_QT514 == "yes"  && "$USE_SYSTEM_QT" == "no" && "$DO_SERVER_COMPONENTS_ONLY" == "no" ]] ; then
        check_if_installed "qt" $QT_VERSION
        if [[ $? == 0 ]] ; then
